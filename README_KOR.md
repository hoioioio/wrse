...
- **관련 분석(Dune)**: [LST/LRT 모니터 – stETH (Ethereum)](https://dune.com/hoin/lst-lrt-monitor-steth-ethereum) (Cloudflare 정책에 따라 비로그인 접근이 차단될 수 있습니다)
...
| :---------------- | :-------------------------: | :------------------------------: |
| **누적 수익률**        |         **+55.74%**         |              +43.10%             |
| **연 환산(CAGR)**    |          **11.74%**         |               9.39%              |
| **최대 낙폭(MDD)**    |         **-11.99%**         |              -12.73%             |
| **샤프 지수(Sharpe)** |           **0.78**          |               0.64               |
...
📁 C:\wrse\
 ...
 ├── 📁 wrse_rust/                 # (선택) 실행 로직 Rust 포트
 ├── cli.py                     # 커맨드라인 실행 인터페이스
 ├── report.py                  # 그래프 테어시트 & JSON 생성기
 ├── verify_portfolio.py        # 산출물 정합성 체크
 └── requirements.txt
```
